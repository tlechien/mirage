...
class BtHCIDevice(wireless.Device):
    ...
    def _initBT(self):
        ...
        self._internalCommand(HCI_Cmd_Set_Event_Mask(
            mask=b"\xFF\xFF\xFB\xFF\x07\xF8\xBF\x3D"))
            ...
            if response.type == 0x04 and response.code == 0xe and response.opcode == cmd.opcode:
                if response.status != 0:
                    raise BluetoothCommandError(
                        "Command %x failed with %x" % (cmd.opcode, response.status))
                return response
    ...
    def _setCurrentHandle(self, handle, address=""):
        if handle != -1:
            ...
            if not found:
                self.handles.append(
                    {"address": address.upper() if address is not None else "", "handle": handle})
    ...
    def setLocalName(self, name):
        ...
        '''
        self._enterCommandMode()
        self._internalCommand(HCI_Cmd_Write_Local_Name(
            local_name=name, padding="\x00"*(150 - len(name))))
        self._exitCommandMode()
    ...
    def _getManufacturerId(self):
        self._enterCommandMode()
        response = self._internalCommand(
    ...
    def setAddress(self, address):
        ...
        if self.isAddressChangeable():
            ...
            if manufacturer not in COMPATIBLE_VENDORS:
                io.fail(
                    "The vendor has not provided a way to modify the BD Address.")
                success = False
            elif manufacturer == 10:  # Cambridge Silicon Radio
                self._internalCommand(HCI_Cmd_CSR_Write_BD_Address(
                    addr=address), noResponse=True)
                ...
                self._internalCommand(
                    modificationPackets[manufacturer](addr=address))
                self._internalCommand(HCI_Cmd_Reset())
                io.success("BD Address successfully modified !")
                success = True
            self._exitCommandMode()
...
class BluetoothEmitter(wireless.Emitter):
    ...
    def convert(self, p):
        if isinstance(p, BluetoothConnect):
            p.packet = HCI_Hdr()/HCI_Command_Hdr()/HCI_Cmd_Create_Connection(addr=p.address, packet_type=p.packetType,
                                                                             page_scan_repetition_mode=p.pageScanRepetitionMode, clock_offset=p.clockOffset, allow_role_switch=p.allowRoleSwitch)
        elif isinstance(p, BluetoothDisconnect):
            handle = p.connectionHandle if p.connectionHandle != - \
                1 else self.device.getCurrentHandle()
            p.packet = HCI_Hdr()/HCI_Command_Hdr()/HCI_Cmd_Disconnect(handle=handle)
        elif isinstance(p, BluetoothInquiry):
            p.packet = HCI_Hdr()/HCI_Command_Hdr()/HCI_Cmd_Inquiry(lap=p.lap,
                                                                   num_response=p.numResponses, inquiry_length=p.inquiryLength)
        elif isinstance(p, BluetoothWriteExtendedInquiryResponse):
            p.packet = HCI_Hdr()/HCI_Command_Hdr()/HCI_Cmd_Write_Extended_Inquiry_Response(
                fec_required=0x00 if not p.fecRequired else 0x01, data=p.data)
        elif isinstance(p, BluetoothRemoteNameRequest):
            p.packet = HCI_Hdr()/HCI_Command_Hdr()/HCI_Cmd_Remote_Name_Request(addr=p.address,
                                                                               page_scan_repetition_mode=p.pageScanRepetitionMode)
        elif isinstance(p, BluetoothWriteScanEnable):
            if p.scanEnable is not None:
                p.packet = HCI_Hdr()/HCI_Command_Hdr() / \
                    HCI_Cmd_Write_Scan_Enable(scan_enable=p.scanEnable)
            else:
                finalValue = 0
                if p.discoverable:
                    finalValue += 1
                ...
                p.packet = HCI_Hdr()/HCI_Command_Hdr() / \
                    HCI_Cmd_Write_Scan_Enable(scan_enable=finalValue)
        elif isinstance(p, BluetoothAcceptConnectionRequest):
            p.packet = HCI_Hdr()/HCI_Command_Hdr()/HCI_Cmd_Accept_Connection_Request(addr=p.address,
                                                                                     role_switch=0x01 if p.role == "slave" else 0x00)
        elif isinstance(p, BluetoothRejectConnectionRequest):
            p.packet = HCI_Hdr()/HCI_Command_Hdr() / \
                HCI_Cmd_Reject_Connection_Request(
                    addr=p.address, reason=p.reason)
        elif isinstance(p, BluetoothL2CAPConnectionRequest):
            handle = p.connectionHandle if p.connectionHandle != - \
                1 else self.device.getCurrentHandle()
            p.packet = HCI_Hdr()/HCI_ACL_Hdr(handle=handle)/L2CAP_Hdr(cid=1)/L2CAP_CmdHdr() / \
                L2CAP_ConnReq(
                    psm=p.psm, scid=p.scid)  # cid == 1 : L2CAP signaling channel
        elif isinstance(p, BluetoothL2CAPConnectionResponse):
            handle = p.connectionHandle if p.connectionHandle != - \
                1 else self.device.getCurrentHandle()
            p.packet = HCI_Hdr()/HCI_ACL_Hdr(handle=handle)/L2CAP_Hdr(cid=1)/L2CAP_CmdHdr()/L2CAP_ConnResp(dcid=p.dcid,
                                                                                                           scid=p.scid, status=p.status, result=p.result)  # cid == 1 : L2CAP signaling channel
        elif isinstance(p, BluetoothL2CAPInformationRequest):
            handle = p.connectionHandle if p.connectionHandle != - \
                1 else self.device.getCurrentHandle()
            p.packet = HCI_Hdr()/HCI_ACL_Hdr(handle=handle)/L2CAP_Hdr(cid=1)/L2CAP_CmdHdr() / \
                L2CAP_InfoReq(
                    type=p.type, data=p.data)  # cid == 1 : L2CAP signaling channel
        elif isinstance(p, BluetoothL2CAPInformationResponse):
            handle = p.connectionHandle if p.connectionHandle != - \
                1 else self.device.getCurrentHandle()
            p.packet = HCI_Hdr()/HCI_ACL_Hdr(handle=handle)/L2CAP_Hdr(cid=1)/L2CAP_CmdHdr() / \
                L2CAP_InfoResp(type=p.type, result=p.result,
                               data=p.data)  # cid == 1 : L2CAP signaling channel
        elif isinstance(p, BluetoothL2CAPConfigurationRequest):
            handle = p.connectionHandle if p.connectionHandle != - \
                1 else self.device.getCurrentHandle()
            p.packet = HCI_Hdr()/HCI_ACL_Hdr(handle=handle)/L2CAP_Hdr(cid=1)/L2CAP_CmdHdr() / \
                L2CAP_ConfReq(dcid=p.dcid, flags=p.flags) / \
                p.data  # cid == 1 : L2CAP signaling channel
        elif isinstance(p, BluetoothL2CAPConfigurationResponse):
            handle = p.connectionHandle if p.connectionHandle != - \
                1 else self.device.getCurrentHandle()
            p.packet = HCI_Hdr()/HCI_ACL_Hdr(handle=handle)/L2CAP_Hdr(cid=1)/L2CAP_CmdHdr() / \
                L2CAP_ConfResp(scid=p.scid, flags=p.flags, result=p.result) / \
                p.data  # cid == 1 : L2CAP signaling channel
        ...
        elif isinstance(p, BluetoothSimplePairingMode):
            p.packet = HCI_Hdr()/HCI_Command_Hdr()/HCI_Cmd_Write_Simple_Pairing_Mode(mode=p.mode)
        elif isinstance(p, BluetoothIOCapabilityReply):
            p.packet = HCI_Hdr()/HCI_Command_Hdr()/HCI_Cmd_IO_Capability_Request_Reply(addr=p.address,
                                                                                       io_capability=p.IOCapability,
                                                                                       oob_data_present=p.OOBDataPresent,
                                                                                       mitm_protection_required=p.AuthenticationRequirements)
        elif isinstance(p, BluetoothUserConfirmationReply):
            p.packet = HCI_Hdr()/HCI_Command_Hdr() / \
                HCI_Cmd_User_Confirmation_Request_Reply(addr=p.address)
        elif isinstance(p, BluetoothPasskeyReply):
            p.packet = HCI_Hdr()/HCI_Command_Hdr() / \
                HCI_Cmd_Passkey_Request_Reply(
                    addr=p.address, passkey=p.passkey)
        return p.packet
        ...
        if "hci" in self.interface:
            if HCI_Evt_Extended_Inquiry_Result in packet:
                ...
                if packet.status == 0x00:
                    ...
                    io.info('Uploading connection handle : ' +
                            str(self.device.getCurrentHandle()))
                ...
                return BluetoothCommandStatus(status=packet.status, numberOfAllowedCmdPackets=packet.number, opcode=packet.opcode)
            elif HCI_Event_Command_Complete in packet:
                return BluetoothCommandComplete(status=packet.status, numberOfAllowedCmdPackets=packet.number, opcode=packet.opcode)
            elif HCI_Evt_Link_Key_Request in packet:
                return BluetoothLinkKeyRequest(address=packet.addr)
            elif HCI_Evt_IO_Capability_Request in packet:
                return BluetoothIOCapabilityRequest(address=packet.addr)
            elif HCI_Evt_User_Confirmation_Request in packet:
                return BluetoothUserConfirmationRequest(address=packet.addr, value=packet.numeric_value)
            elif HCI_Evt_Passkey_Request in packet:
                return BluetoothPasskeyRequest(address=packet.addr)
            elif HCI_Evt_Authentication_Complete in packet:
                ...
                if packet.status == 0x00:
                    self.device._removeConnectionHandle(packet.handle)
                    io.info('Removing connection handle : ' +
                            str(packet.handle))
