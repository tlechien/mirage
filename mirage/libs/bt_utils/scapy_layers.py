...
class HCI_Cmd_Write_Extended_Inquiry_Response(Packet):
    ...
    fields_desc = [	ByteEnumField("fec_required", 0x00, {0x00: "false", 0x01: "true"}),
...
class HCI_Evt_Extended_Inquiry_Result(Packet):
    ...
    fields_desc = [ByteField("num_response", 5),
                   ...
                   ByteEnumField("page_scan_repetition_mode",
                                 0x01, {0x01: "R1"}),
...
class HCI_Cmd_Write_Inquiry_Mode(Packet):
    ...
    fields_desc = [ByteEnumField("inquiry_mode", 0x02, {
                                 0x02: "Results with RSSI  / Extended"})]
...
class HCI_Evt_Connection_Complete(Packet):
    ...
    fields_desc = [	ByteEnumField("status", 0x00, bluetooth_error_codes),
                    LEShortField("handle", 0),
                    LEMACField("addr", None),
                    ByteEnumField("link_type", 0x01, {0x01: "ACL connection"}),
                    ByteEnumField("encryption_mode", 0x00, {
...
class HCI_Evt_Remote_Name_Request_Complete(Packet):
    ...
    fields_desc = [
        ByteEnumField("status", 0x00, bluetooth_error_codes),
        LEMACField("addr", None),
...
# There is already a scapy struct for Command Status Packets.
# class HCI_Evt_Authentication_Request(Packet):
#    name = "Command Status (Authentication Requested)"
#    fields_desc = [
#        ByteEnumField("status", 0x00, {**bluetooth_error_codes, 0x00: 'Pending.'}),
#        ByteField("number_of_allowed_command_packets", None),
#        LEShortField("opcode", 0),
#    ]
#
# class HCI_Evt_Request_Complete(Packet):
#    name = "Command Status (Request Reply)"
#    fields_desc = [
#        ByteField("number_of_allowed_command_packets", None),
#        LEShortField("opcode", 0),
#        ByteEnumField("status", 0x00, bluetooth_error_codes),
...
class HCI_Evt_Authentication_Complete(Packet):
...
class HCI_Evt_Link_Key_Request(Packet):
    name = "Link Key Request"
    fields_desc = [LEMACField("addr", None)]


class HCI_Evt_IO_Capability_Request(Packet):
...
class HCI_Evt_User_Confirmation_Request(Packet):
    name = "User Confirmation Request"
    fields_desc = [
        LEMACField('addr', None),
        IntField('numeric_value', None)
    ]
...
class HCI_Connection_Handle(Packet):
    name = "HCI Connection Handle"
    fields_desc = [LEShortField("handle", 0)]


class HCI_BD_Address(Packet):
    name = "BD Address"
    fields_desc = [LEMACField("addr", None)]
...
# Those 3 are pretty much the same.
# class HCI_Evt_Link_Key_Request_Complete(Packet):
#        name = "Command Status (Authentication Request Reply)"
#        fields_desc = [
#		ByteField("number_of_allowed_command_packets",None),
#		LEShortField("opcode", 0),
#                ByteEnumField("status", 0x00, {0x00:"success",0x04:"page_timeout",0x02:"unknown_connection_identifier"}),
#		LEMACField('addr',None),
#        ]
#
# class HCI_Evt_IO_Capability_Request_Complete(Packet):
#        name = "Command Complete (IO Capability Request Reply)"
#        fields_desc = [
#		ByteField("number_of_allowed_command_packets",None),
#		LEShortField("opcode", 0),
#                ByteEnumField("status", 0x00, {0x00:"success",0x04:"page_timeout",0x02:"unknown_connection_identifier"}),
...
#        ]
#
# class HCI_Evt_User_Confirmation_Request_Complete(Packet):
#        name = "Command Complete (User Confirmation Request Reply)"
#        fields_desc = [
#		ByteField("number_of_allowed_command_packets",None),
#		LEShortField("opcode", 0),
#                ByteEnumField("status", 0x00, {0x00:"success",0x04:"page_timeout",0x02:"unknown_connection_identifier"}),
...
class HCI_Evt_Link_Key_Notification(Packet):
...
class Data_Element_Hdr(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(ByteField("additional_8_bits_size",
                         None), lambda pkt: pkt.size == 5),
        ConditionalField(ShortField("additional_16_bits_size",
...
class Data_Element_Unsigned_Integer(Data_Element_Value):
    ...
    fields_desc = [
        ...
        ConditionalField(ByteField("value_1", None),
                         lambda pkt:pkt.data_size == 1),
        ConditionalField(ShortField("value_2", None),
                         lambda pkt:pkt.data_size == 2),
        ConditionalField(IntField("value_4", None),
                         lambda pkt:pkt.data_size == 4),
        ConditionalField(LongField("value_8", None),
                         lambda pkt:pkt.data_size == 8),
        ConditionalField(StrFixedLenField('value_16', None,
                         length=16), lambda pkt:pkt.data_size == 16)
    ]
...
class Data_Element_Signed_Integer(Data_Element_Value):
    ...
    fields_desc = [IntField("data_size", None),
                   ConditionalField(ByteField("value_1", None),
                                    lambda pkt:pkt.data_size == 1),
                   ConditionalField(SignedShortField(
                       "value_2", None), lambda pkt:pkt.data_size == 2),
                   ConditionalField(SignedIntField(
                       "value_4", None), lambda pkt:pkt.data_size == 4),
                   ConditionalField(LongField("value_8", None),
                                    lambda pkt:pkt.data_size == 8),
...
class Data_Element_UUID(Data_Element_Value):
    ...
    fields_desc = [IntField("data_size", None),
                   ConditionalField(XLEShortField("value_2", None),
                                    lambda pkt:pkt.data_size == 2),
                   ConditionalField(XLEIntField("value_4", None),
                                    lambda pkt:pkt.data_size == 4),
                   ConditionalField(XLELongField("value_8", None),
                                    lambda pkt:pkt.data_size == 8)
                   ]
...
class Data_Element_String(Data_Element_Value):
    ...
    fields_desc = [IntField("data_size", None),
                   StrLenField("value", None,
                               length_from=lambda pkt: pkt.data_size)
                   ]
...
class SDP_Service_Search_Request(Packet):
    ...
    fields_desc = [
        ...
        StrFixedLenField("continuation_state", None,
                         length_from=lambda pkt: pkt.continuation_state_count)
    ]
...
class SDP_Service_Search_Response(Packet):
    ...
    fields_desc = [
        ...
        FieldListField("service_record_handle_list", [], IntField,
                       count_from=lambda pkt:pkt.current_service_record_count),
        ByteEnumField("continuation_state_count", None, {0x00: "no"}),
        StrFixedLenField("continuation_state", None,
                         length_from=lambda pkt: pkt.continuation_state_count)
    ]
...
class SDP_Service_Attribute_Response(Packet):
    ...
    fields_desc = [
        ...
        PacketListField("attribute_list", [], Data_Element_Hdr,
                        length_from=lambda pkt:pkt.attribute_list_byte_count),
        ByteEnumField("continuation_state_count", None, {0x00: "no"}),
        StrFixedLenField("continuation_state", None,
                         length_from=lambda pkt: pkt.continuation_state_count)
    ]
