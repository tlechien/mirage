...
class BtUbertoothDevice(wireless.Device):
    ...
    def resetUbertooth(cls, index=0):
        ...
        '''
        try:
            device = list(usb.core.find(idVendor=UBERTOOTH_ID_VENDOR,
                          idProduct=UBERTOOTH_ID_PRODUCT, find_all=True))[index]
    ...
    def _getModulation(self):
        modulation = self.ubertooth.ctrl_transfer(
            CTRL_IN, UBERTOOTH_GET_MOD, 0, 0, 1)
        modulation = struct.unpack('b', modulation)[0]
        return modulation

    def _setModulation(self, modulation=MOD_BT_LOW_ENERGY):
        self.ubertooth.ctrl_transfer(
            CTRL_OUT, UBERTOOTH_SET_MOD, modulation, 0)

    def _getSerial(self):
        serial = self.ubertooth.ctrl_transfer(
            CTRL_IN, UBERTOOTH_GET_SERIAL, 0, 0, 17)
    ...
    def _setCRCChecking(self, enable=True):
        self.ubertooth.ctrl_transfer(
            CTRL_OUT, UBERTOOTH_SET_CRC_VERIFY, (1 if enable else 0), 0)
    ...
    def init(self):
        ...
        try:
            self.ubertooth = list(
                usb.core.find(idVendor=UBERTOOTH_ID_VENDOR,
                              idProduct=UBERTOOTH_ID_PRODUCT, find_all=True)
            )[self.index]

            self.version = '{0:x}.{1:x}'.format(
                (self.ubertooth.bcdDevice >> 8) & 0x0FF, self.ubertooth.bcdDevice & 0x0FF)
        except:
            self.ubertooth = None
