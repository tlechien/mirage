...
class HCI_Cmd_CSR_Write_BD_Address(Packet):
    ...
    def post_build(self, p, pay):
        payload = bytearray(
            b"\xc2\x02\x00\x0c\x00\x11G\x03p\x00\x00\x01\x00\x04\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00")
...
class HCI_Cmd_CSR_Reset(Packet):
    name = "CSR Write BD Address"
    fields_desc = [StrField(
        "bytes", b"\xc2\x02\x00\t\x00\x00\x00\x01@\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00")]
...
bind_layers(HCI_Command_Hdr,
            HCI_Cmd_Read_Local_Version_Information, 				opcode=0x1001)
bind_layers(HCI_Event_Command_Complete,
            HCI_Cmd_Complete_Read_Local_Version_Information, 	opcode=0x1001)
...
bind_layers(HCI_Command_Hdr,
            HCI_Cmd_Ericsson_Write_BD_Address, 				opcode=0xfc0d)
